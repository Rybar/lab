function init() {
    ...
    .addNumber("seed", 0, 10000000, 1019, .0001)
    .addRange("pattern width", 0, 16, 4, 1)
    loop();
}
...
function draw(dt){
    ...
    spr(0,0, 4*tiles-1, 4*tiles-1, 0,       0,       false, false);    
    spr(0,0, 4*tiles-1, 4*tiles-1, 4*tiles, 0,       true, false);
    spr(0,0, 4*tiles-1, 4*tiles-1, 0,       4*tiles, false, true);
    spr(0,0, 4*tiles-1, 4*tiles-1, 4*tiles, 4*tiles, true, true);
    
    renderSource = BUFFER2;
    renderTarget = SCREEN;
    sx = WIDTH/2 - 4*tiles*scale;
    sy = HEIGHT/2 - 4*tiles*scale;
    size = 4*tiles*2*scale+1;
